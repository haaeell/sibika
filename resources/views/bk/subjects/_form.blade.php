...
<div class="grid gap-4 md:grid-cols-2">
    ...
    <x-form.select name="category" label="Kategori" icon="fa-solid fa-layer-group">
        @foreach (['general' => 'Umum', 'tka_mandatory' => 'TKA Wajib', 'tka_optional' => 'TKA Pilihan'] as $value => $label)
            <option value="{{ $value }}" @selected(old('category', $subject->category ?: 'general') === $value)>{{ $label }}</option>
        @endforeach
    </x-form.select>
</div>
